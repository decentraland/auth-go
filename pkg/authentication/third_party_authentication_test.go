...
const validEntityId = "the auth server"
...
func TestValidateAccessToken(t *testing.T) {
	for _, tc := range validateAccessTokenTC {
		t.Run(tc.name, func(t *testing.T) {
			...
			k, _ := utils.ReadPublicKey(tc.entity.entityPubKey)
			entities[tc.entity.entityId] = k

			err := validateAccessToken(tc.accessToken, entities, tc.ephemeralKey)
...
var validateAccessTokenTC = []accessTokenTestCase{
	{
		...
		entity: entityData{
			entityPubKey: validEntityPubKey,
	...
	},
	{
		...
		resultEvaluation: evaluateError("unknown entity"),
	},
}
